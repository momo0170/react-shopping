...
// 장바구니 데이터 추가
export async function addCartData(uid, cartData, uniqueID) {
  const modifiedData = JSON.parse(JSON.stringify(cartData));
  delete modifiedData.id;
  console.log(modifiedData);

  return set(ref(db, `cart/${uid}/${uniqueID}`), {
    ...modifiedData,
  });
}
...
// 장바구니 데이터 삭제
export async function deleteCartData(uid, uniqueID) {
  return remove(ref(db, `cart/${uid}/${uniqueID}`)) //
    .then(() => {
      console.log(`${uniqueID} 제품이 삭제되었습니다.`);
    });
}
